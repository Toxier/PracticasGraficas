...
public:
	void Carga(WCHAR nombre[])
	{
	   ...
	   try
	   {
		   dir_imagen = new unsigned char [tamao]; //[tama�o*1000000]; ponganle esto como prueba
		   primero=dir_imagen;
	   }
		   ...
		   switch (msgboxID)
			{
				...
				case IDCANCEL:
					exit(0);
					break;
				//un nuevo intento con una textura mas peque�o en todo caso
				//si no alcanza pa los tacos tonces pa la tortilla, que me pasa? ya com�?
				case IDTRYAGAIN:
					try{
					dir_imagen = new unsigned char [tamao];
					primero=dir_imagen;
					}
